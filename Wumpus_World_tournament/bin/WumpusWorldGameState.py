...
class WumpusGameState():
    ...
    def __init__(self, dim, wumpusX, wumpusY, goldX, goldY, pits, Agent):
            self._rows = dim
            self._columns = dim
            self.board = [[set() for j in range(self._columns)] for i in range(self._rows)] 
            self._wumpus = self._set_obj('Wumpus', wumpusX, wumpusY, 'Stench')        
            self._gold = self._set_obj('Glitter', goldX, goldY, False)
            
            for pit in pits:
                self._set_obj('Pit', pit[0], pit[1], 'Breeze')
    ...
    def _set_obj(self, obj, x, y, around_obj = False):
        """
        Given an object to set and another object to set around
        the first object, sets the object in a specific spot and
        surrounds it with the other object. Example: Wumpus
        and Stench
        """
        self.board[x][y].add(obj)
...
if __name__ == '__main__':
    ...
    game = WumpusGameState(int(size), int(size), Agento)
    game.start()
